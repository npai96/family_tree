...
def create_person(
    ...
    with get_conn() as conn:
        ...
        row = fetch_one(conn, "SELECT * FROM persons WHERE id = ?", (person_id,))
        _log_audit(
            conn,
            circle_id=circle_id,
            actor_user_id=actor_user_id,
...
def create_person_place(
    ...
    with get_conn() as conn:
        ...
        row = fetch_one(conn, "SELECT * FROM person_places WHERE id = ?", (place_id,))
...
def create_relationship(
    ...
    with get_conn() as conn:
        ...
        row = fetch_one(conn, "SELECT * FROM relationships WHERE id = ?", (rel_id,))
        _log_audit(
            conn,
            circle_id=circle_id,
            actor_user_id=actor_user_id,
...
def update_relationship(
    ...
    with get_conn() as conn:
        ...
        try:
            ...
            raise HTTPException(status_code=409, detail=str(err)) from err
        row = fetch_one(conn, "SELECT * FROM relationships WHERE id = ?", (relationship_id,))
        _log_audit(
            conn,
            circle_id=circle_id,
            actor_user_id=actor_user_id,
...
def create_context_event(
    ...
    with get_conn() as conn:
        ...
        row = fetch_one(conn, "SELECT * FROM context_events WHERE id = ?", (event_id,))
